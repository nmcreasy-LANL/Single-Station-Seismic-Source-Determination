...
def plot_polarization_event_noise(st, 
    ...
    for tr_Z, tr_N, tr_E in zip(st_Z, st_N, st_E):
        #print(tr_Z.stats.npts,winlen*4)
        if tr_Z.stats.npts < winlen * 4:
            print('window length is bad')
            continue
        else:
            print('good')
    ...
    for i,ax in enumerate(axes0[:, 0]):
        ax.grid(which='both', axis='x') #turn on the grid for the time ticks
    ...
    for ax in axes0[1:, 1:].flatten():
        ax.grid(which='both', axis='x', linewidth=0.2, color='grey')
    ...
    plt.close('all')
## saving a file - NEALA ADDED (modified to save both P and S wave data)
    with open(fname+'out.pkl', 'wb') as f:
            # Save both P-wave and S-wave KDE data for backazimuth extraction
            pickle.dump({'P': kde_dataframe_P, 'S': kde_dataframe_S}, f)
        #f.close()
###
...
def plot_baz_from_p_and_s(baz_KDE, inc_KDE,
    ...
    maxInc = maxIndex[0][0]
    maxbaz = maxIndex[1][0]
    #print('P from S-wave',maxbaz,maxInc)
    ...
    if True in twodmask_S[0]: #if second pick is available, some part of the bool mask must be true
        ...
        print('P from S-wave',np.rad2deg(baz_range[maxbaz]+half_baz_step),inc_range[maxInc]+0.5*inc_step)
        print('P from P-wave',np.rad2deg(baz_KDE),np.rad2deg(inc_KDE))
#### NEALA ADDED ###
...
def fwhm_error_from_kde(xs, ys, index):
    ...
    for k in range(index_local, 0, -1): #backwards
       # print(indexes_ymax[k-1],indexes_ymax[k]-1)
        if indexes_ymax[k-1] < indexes_ymax[k]-1:
            index_low = indexes_ymax[k]
            break
        elif k == 1: #if there is only one peak
            index_low = indexes_ymax[0]
    #print(xs[0]), #print(index_local)
    ##NEALA ADDDED , can make the following 0 for synthetics
