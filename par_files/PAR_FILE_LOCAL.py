...
# -*- coding: utf-8 -*-
"""
PAR_FILE_LOCAL.py - Parameter file for LOCAL 2015 event analysis

This file contains all configuration parameters for analyzing the LOCAL event.
To run analysis with this parameter file:
    python sparse_data_location_analysis.py --par_file par_files/PAR_FILE_LOCAL.py
...
FILES = [
    'II.BFO.00.BHE.M.2013.346.005000.SAC',  # East component
    'II.BFO.00.BHZ.M.2013.346.005000.SAC',  # Vertical component
    'II.BFO.00.BHN.M.2013.346.005000.SAC'   # North component
]
...
EVENT_TIME = "2013-12-12T00:59:18"  # Unknown - set for blind analysis
...
SOURCE_DEPTH_KM = 6    # Update if known in kilometers
DISTANCE_DEG = 1.49       # Update if known distance in degrees
...
TRUE_EVENT_LAT = 47.05  # Update if known (degrees)
TRUE_EVENT_LON = 9.49  # Update if known (degrees)
...
STAGE1_DISTANCE_RANGE = [70, 98]       # Broad initial distance guess (degrees)
STAGE1_DEPTH_RANGE = [0, 700]          # Broad initial depth guess (km)
...
STAGE1_GRID_DISTANCES = np.arange(71, 101, 3)  # 71-98°, 3° spacing (10 points)
...
PICK_UNCERTAINTIES = {
    'P': 0.5,    # P-wave uncertainty (±10s typical)
    'Pn': 0.5,   # PP (surface reflection) uncertainty
    'PP': 0.5,    # S-wave uncertainty
    'Sn': 0.5    # PS (P-to-S conversion) uncertainty
}

# Legacy variables for backward compatibility (auto-generated from PICK_UNCERTAINTIES)
PICK_UNCERTAINTY_P = PICK_UNCERTAINTIES.get('P', 0.5)
PICK_UNCERTAINTY_PP = PICK_UNCERTAINTIES.get('Pn', 0.5)
PICK_UNCERTAINTY_S = PICK_UNCERTAINTIES.get('PP', 0.5)
PICK_UNCERTAINTY_PS = PICK_UNCERTAINTIES.get('Sn', 0.5)
...
POLARIZATION_PARAMS = {
    ...
    't_window_P': [-1, 5],        # [before, after] P pick for analysis window
    't_window_S': [-1, 5],        # [before, after] S pick for analysis window
    ...
    'fmin': 1/2,                    # Minimum frequency (Hz) = 50s period
    'fmax': 2,                    # Maximum frequency (Hz) = 10s period
    'f_band_density': (1/50, 4),  # Frequency band for BAZ estimation (Hz)
...
NUM_BANDS = 6               # Number of overlapping frequency bands
PERIOD_MIN = 100            # Minimum period (seconds)
PERIOD_MAX = 200            # Maximum period (seconds)
OVERLAP_PERCENT = 88        # Band overlap percentage (high overlap = more data)
